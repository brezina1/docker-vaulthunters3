...
if ! [[ -f "Vault-Hunters-3rd-Edition-3.14-server-files.zip" ]]; then
  rm -fr config mods defaultconfigs scripts forge-*.jar start.sh *Server.zip
	curl -Lo 'Vault-Hunters-3rd-Edition-3.14-server-files.zip' 'https://edge.forgecdn.net/files/5394/896/Vault-Hunters-3rd-Edition-3.14-server-files.zip' && unzip -u -o 'Vault-Hunters-3rd-Edition-3.14-server-files.zip' -d /data
