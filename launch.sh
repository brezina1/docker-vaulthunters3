...
if ! [[ -f "Vault-Hunters-3rd-Edition-3.17.1.0-server-files.zip" ]]; then
  rm -fr config mods defaultconfigs scripts forge-*.jar start.sh *.zip
	curl -Lo 'Vault-Hunters-3rd-Edition-3.17.1.0-server-files.zip' 'https://edge.forgecdn.net/files/6377/659/Vault-Hunters-3rd-Edition-3.17.1.0-server-files.zip' && unzip -u -o 'Vault-Hunters-3rd-Edition-3.17.1.0-server-files.zip' -d /data
